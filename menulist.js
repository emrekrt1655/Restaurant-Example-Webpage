...
function displayCategory(){
    ...
    menu.forEach((items)=>{
        ...
        if (categories.indexOf(items.category)=== -1){
            categories.push(items.category);
        }; 
    });
    categories.forEach((part) =>{
        categoryContent += `<button type="button" class="filter-btn" data-id=${part}>${part}</button>`
    });
    
  sectionButton.innerHTML = categoryContent;

    const buttons = document.querySelectorAll(".filter-btn");

    buttons.forEach((i) => {
        i.addEventListener("click", (e)=>{
            const selectedItem = e.target.getAttribute("data-id");
